...
function render(view) {
  ...
  if (view === 'checkout') hydrateCheckoutPage();
}
...
function hydrateCheckoutPage() {
  api('/cart').then((cart) => {
    ...
    if (summary) summary.innerHTML = `${cart.items.map((item) => `<div class="summary-row"><span>${item.product.shortName} × ${item.quantity}</span><strong>₹${item.lineTotal.toLocaleString('en-IN')}</strong></div>`).join('')}<div class="summary-row"><span>Express delivery</span><strong>Free</strong></div><div class="summary-row total"><span>Total</span><strong>₹${cart.total.toLocaleString('en-IN')}</strong></div>`;
    document.querySelector('#checkoutForm')?.addEventListener('submit', (event) => {
      event.preventDefault();
      const customer = { name: document.querySelector('#checkoutName').value.trim(), phone: document.querySelector('#checkoutPhone').value.trim(), address: `${document.querySelector('#checkoutAddress').value.trim()}, ${document.querySelector('#checkoutCity').value.trim()} - ${document.querySelector('#checkoutPin').value.trim()}` };
      const paymentMethod = document.querySelector('input[name="paymentMethod"]:checked').value;
      api('/orders', { method: 'POST', body: JSON.stringify({ customer, paymentMethod }) }).then(({ order }) => { showToast(`Order ${order.id} confirmed`); setTimeout(() => { window.location.href = 'index.html'; }, 900); }).catch((error) => showToast(error.message));
    });
